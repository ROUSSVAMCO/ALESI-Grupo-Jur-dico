...
def clientes():
    ...
    return render("""<div class="card no-print no-imprimir"><h2>Registrar cliente</h2><p class="small">Puedes registrar el cliente una sola vez y después crearle varios expedientes/asuntos con montos distintos.</p><form method="post"><label>Nombre</label><input name="nombre" required><label>Teléfono</label><input name="telefono"><label>Correo</label><input name="correo"><label>RFC</label><input name="rfc"><label>CURP</label><input name="curp"><label>Materia jurídica principal</label><select name="materia">{% for m in materias %}<option>{{m}}</option>{% endfor %}</select><label>Tipo de asunto principal</label><select name="tipo_asunto">{% for t in tipos %}<option>{{t.nombre}}</option>{% endfor %}</select><label>Cobro total general del cliente</label><input type="number" step="0.01" name="cobro_total"><label>Moneda</label><select name="moneda_cobro"><option>MXN</option><option>USD</option></select><label>Dirección</label><textarea name="direccion"></textarea><label>Observaciones</label><textarea name="observaciones"></textarea><button>Guardar</button></form></div><div class="card print-list"><h2>Lista de clientes <button class="no-print" onclick="window.print()">Imprimir lista</button></h2><table><tr><th>Cliente</th><th>Teléfono</th><th>Materia</th><th>Tipo asunto</th><th>Cobro general</th><th class="no-print">Acción</th></tr>{% for c in datos %}<tr><td><a href="/cliente/{{c.id}}">{{c.nombre}}</a></td><td>{{c.telefono}}</td><td>{{c.materia}}</td><td>{{c.tipo_asunto}}</td><td>{{c.moneda_cobro}} {{c.cobro_total}}</td><td class="no-print"><a class="btn btn2" href="/cliente/{{c.id}}">Ver</a> <a class="btn" href="/editar-cliente/{{c.id}}">Editar</a></td></tr>{% endfor %}</table></div>""", materias=MATERIAS, tipos=TipoAsunto.query.order_by(TipoAsunto.materia, TipoAsunto.nombre).all(), datos=Cliente.query.order_by(Cliente.nombre).all())
...
def cliente(id):
    ...
    total_expedientes = sum([(e.cobro_total or 0) for e in exps])
    abonado = sum([(p.abono or 0) for p in pagos])

    pagos_por_expediente = []
    for e in exps:
        pagos_exp = Cobranza.query.filter_by(cliente_id=id, expediente_id=e.id).order_by(Cobranza.id.desc()).all()
        abonado_exp = sum([(p.abono or 0) for p in pagos_exp])
        total_exp = e.cobro_total or 0
        saldo_exp = max(total_exp - abonado_exp, 0)
        pagos_por_expediente.append({
            "expediente": e,
            "pagos": pagos_exp,
            "abonado": abonado_exp,
            "saldo": saldo_exp,
            "estatus_pago": "Liquidado" if saldo_exp <= 0 and total_exp > 0 else "Pendiente"
        })

    pagos_sin_expediente = Cobranza.query.filter_by(cliente_id=id, expediente_id=None).order_by(Cobranza.id.desc()).all()

    return render("""
<div class="card print-list">
    <h2>Datos del cliente <button class="no-print" onclick="window.print()">Imprimir reporte del cliente</button></h2>
    <p><b>Nombre:</b> {{c.nombre}}</p>
    <p><b>Teléfono:</b> {{c.telefono}}</p>
    <p><b>Correo:</b> {{c.correo}}</p>
    <p><b>RFC:</b> {{c.rfc}}</p>
    <p><b>CURP:</b> {{c.curp}}</p>
    <p><b>Materia principal:</b> {{c.materia}}</p>
    <p><b>Tipo de asunto principal:</b> {{c.tipo_asunto}}</p>
    <p><b>Cobro general:</b> {{c.moneda_cobro}} {{c.cobro_total}}</p>
    <p><b>Dirección:</b> {{c.direccion}}</p>
    <p><b>Observaciones:</b> {{c.observaciones}}</p>

    <div class="no-print">
        <a class="btn" href="/editar-cliente/{{c.id}}">Editar datos del cliente</a>
        <a class="btn btn2" href="/expedientes">Agregar nuevo asunto / expediente</a>
    </div>
</div>

<div class="card print-list">
    <h2>Lista de expedientes iniciados o concluidos</h2>
    <p>
        <b>Total contratado en expedientes:</b> {{c.moneda_cobro}} {{'%.2f'|format(total_expedientes)}} |
        <b>Total abonado por el cliente:</b> {{'%.2f'|format(abonado)}}
    </p>
    <table>
        <tr>
            <th>Número</th>
            ...
            <th>Tipo</th>
            <th>Estado</th>
            <th>Cobro del expediente</th>
            <th>Total abonado</th>
            <th>Saldo</th>
            <th>Estatus pago</th>
            <th class="no-print">Acción</th>
        </tr>
        {% for item in pagos_por_expediente %}
        <tr>
            <td>{{item.expediente.numero}}</td>
            <td>{{item.expediente.materia}}</td>
            <td>{{item.expediente.tipo_asunto}}</td>
            <td>{{item.expediente.estado}}</td>
            <td>{{item.expediente.moneda_cobro or c.moneda_cobro}} {{'%.2f'|format(item.expediente.cobro_total or 0)}}</td>
            <td>{{item.expediente.moneda_cobro or c.moneda_cobro}} {{'%.2f'|format(item.abonado)}}</td>
            <td>{{item.expediente.moneda_cobro or c.moneda_cobro}} {{'%.2f'|format(item.saldo)}}</td>
            <td>{{item.estatus_pago}}</td>
            <td class="no-print"><a class="btn btn2" href="/expediente/{{item.expediente.id}}">Abrir</a></td>
        </tr>
        {% endfor %}
    </table>
</div>
...
<div class="card print-list">
    <h2>Pagos realizados desglosados por expediente</h2>

    {% for item in pagos_por_expediente %}
        <h3>Expediente {{item.expediente.numero}} - {{item.expediente.tipo_asunto}}</h3>
        <p>
            <b>Estado del expediente:</b> {{item.expediente.estado}} |
            <b>Cobro:</b> {{item.expediente.moneda_cobro or c.moneda_cobro}} {{'%.2f'|format(item.expediente.cobro_total or 0)}} |
            <b>Abonado:</b> {{'%.2f'|format(item.abonado)}} |
            <b>Saldo:</b> {{'%.2f'|format(item.saldo)}}
        </p>
        <table>
            <tr>
                <th>Folio</th>
            ...
            <tr>
                ...
                <td>{{p.concepto}}</td>
                <td>{{p.forma_pago}}</td>
                <td>{{p.moneda}} {{'%.2f'|format(p.abono or 0)}}</td>
                <td>{{p.moneda}} {{'%.2f'|format(p.saldo or 0)}}</td>
                <td>{{p.estatus}}</td>
                <td class="no-print"><a href="/recibo/{{p.id}}">Ver</a> | <a href="/recibo-pdf/{{p.id}}">PDF</a></td>
            </tr>
            {% endfor %}
            {% if not item.pagos %}
            <tr><td colspan="8">Sin pagos registrados para este expediente.</td></tr>
            {% endif %}
        </table>
        <br>
    {% endfor %}

    {% if pagos_sin_expediente %}
        <h3>Pagos sin expediente asignado</h3>
        <table>
            <tr>
                <th>Folio</th>
                <th>Fecha pago</th>
...
""", c=c, pagos=pagos, exps=exps, total_expedientes=total_expedientes, abonado=abonado, pagos_por_expediente=pagos_por_expediente, pagos_sin_expediente=pagos_sin_expediente)
...
def expedientes():
    ...
    if request.method == "POST":
        ...
        nuevo = Expediente(
            ...
            moneda_cobro=request.form.get("moneda_cobro") or (cliente.moneda_cobro if cliente else "MXN"),
    ...
    datos = Expediente.query.order_by(Expediente.id.desc()).all() if admin() or (actual() and actual().rol == "Usuario Oficina") else Expediente.query.filter_by(propietario_id=actual().id).order_by(Expediente.id.desc()).all()
    return render("""<div class="card no-print"><h2>Nuevo expediente / asunto</h2><p class="small">Puedes seleccionar el mismo cliente para varios asuntos y poner un cobro diferente por cada expediente.</p><form method="post"><label>Cliente</label><select name="cliente_id" onchange="llenarClienteExpediente(this)"><option value="">Sin cliente</option>{% for cliente in clientes %}<option value="{{cliente.id}}" data-moneda="{{cliente.moneda_cobro or 'MXN'}}" data-total="{{cliente.cobro_total or 0}}">{{cliente.nombre}}</option>{% endfor %}</select><label>Número</label><input name="numero" required><label>Materia</label><select name="materia">{% for m in materias %}<option>{{m}}</option>{% endfor %}</select><label>Tipo de asunto</label><select name="tipo_asunto">{% for t in tipos %}<option>{{t.nombre}}</option>{% endfor %}</select><label>Cobro de este expediente / asunto</label><input id="cobro_total_exp" type="number" step="0.01" name="cobro_total"><label>Moneda del expediente</label><select id="moneda_cobro_exp" name="moneda_cobro"><option>MXN</option><option>USD</option></select><label>Autoridad</label><input name="autoridad"><label>Actor</label><input name="actor"><label>Demandado</label><input name="demandado"><label>Estado</label><select name="estado"><option>Nuevo</option><option>En trámite</option><option>Pendiente de acuerdo</option><option>Pendiente de audiencia</option><option>Concluido</option><option>Archivado</option><option>Urgente</option></select><label>Prioridad</label><select name="prioridad"><option>Baja</option><option selected>Media</option><option>Alta</option></select><label>Responsable</label><input name="responsable"><label>Fecha inicio</label><input type="date" name="fecha_inicio"><label>Observaciones</label><textarea name="observaciones"></textarea><button>Guardar</button></form><script>function llenarClienteExpediente(sel){var o=sel.options[sel.selectedIndex];if(o.dataset.moneda){document.getElementById('moneda_cobro_exp').value=o.dataset.moneda;} if(o.dataset.total){document.getElementById('cobro_total_exp').value=o.dataset.total;}}</script></div><div class="card print-list"><h2>Lista de expedientes cargados <button class="no-print" onclick="window.print()">Imprimir lista</button></h2><table><tr><th>Número</th><th>Cliente</th><th>Materia</th><th>Tipo asunto</th><th>Estado</th><th>Cobro</th><th class="no-print">Acción</th></tr>{% for expediente in datos %}<tr><td>{{expediente.numero}}</td><td>{{expediente.cliente.nombre if expediente.cliente else ""}}</td><td>{{expediente.materia}}</td><td>{{expediente.tipo_asunto}}</td><td>{{expediente.estado}}</td><td>{{expediente.moneda_cobro}} {{expediente.cobro_total}}</td><td class="no-print"><a class="btn btn2" href="/expediente/{{expediente.id}}">Abrir</a></td></tr>{% endfor %}</table></div>""", clientes=Cliente.query.order_by(Cliente.nombre).all(), materias=MATERIAS, tipos=TipoAsunto.query.order_by(TipoAsunto.materia, TipoAsunto.nombre).all(), datos=datos)
...
def mis_expedientes():
    if not req():
        return redirect("/login")
    u = actual()
    if admin() or u.rol == "Usuario Oficina":
...
def cobranza():
    ...
    if request.method == "POST":
        ...
        if expediente and total <= 0:
            total = expediente.cobro_total or 0
        elif cliente and total <= 0:
            total = cliente.cobro_total or 0
        abono = float(request.form["abono"] or 0)
        if expediente:
            pagos_previos = db.session.query(db.func.sum(Cobranza.abono)).filter_by(expediente_id=expediente.id).scalar() or 0
        elif cliente:
            pagos_previos = db.session.query(db.func.sum(Cobranza.abono)).filter_by(cliente_id=cliente.id).scalar() or 0
        else:
            pagos_previos = 0
        saldo = max(total - pagos_previos - abono, 0)
        folio = "ALESI-" + datetime.now().strftime("%Y%m%d%H%M%S")
        p = Cobranza(
            folio=folio, cliente_id=cliente_id, expediente_id=expediente_id,
    ...
    expedientes_activos = Expediente.query.filter(~Expediente.estado.in_(["Concluido", "Archivado"])).order_by(Expediente.numero).all()
    return render("""<div class="card no-print no-imprimir"><h2>Registrar cobranza</h2><form method="post" enctype="multipart/form-data"><label>Cliente</label><select name="cliente_id" id="cliente_id" onchange="filtrarExpedientesCobranza()"><option value="">Seleccione</option>{% for c in clientes_info %}<option value="{{c.id}}" data-total="{{c.cobro_total}}" data-moneda="{{c.moneda}}" data-deuda="{{c.deuda}}">{{c.nombre}} | Cobro general: {{c.moneda}} {{c.cobro_total}} | Debe general: {{c.deuda}}</option>{% endfor %}</select><label>Expediente activo del cliente</label><select name="expediente_id" id="expediente_id" onchange="llenarMontoExpediente()"><option value="">Sin expediente</option>{% for e in expedientes %}<option value="{{e.id}}" data-cliente="{{e.cliente_id or ''}}" data-total="{{e.cobro_total or 0}}" data-moneda="{{e.moneda_cobro or 'MXN'}}">{{e.numero}} | {{e.cliente.nombre if e.cliente else 'Sin cliente'}} | {{e.moneda_cobro or 'MXN'}} {{e.cobro_total or 0}}</option>{% endfor %}</select><label>Concepto</label><select name="concepto"><option>Honorarios</option><option>Abono</option><option>Pago trámite institucional</option><option>Copias certificadas</option><option>Notario</option><option>Viáticos</option><option>Derechos</option><option>Gastos administrativos</option><option>Otro</option></select><label>Descripción</label><textarea name="descripcion"></textarea><label>Total cobrado / pactado</label><input id="monto_total" type="number" step="0.01" name="monto_total"><label>Abono que pagará</label><input type="number" step="0.01" name="abono"><label>Deuda actual antes del abono</label><input id="deuda_actual" disabled><label>Moneda</label><select id="moneda" name="moneda"><option>MXN</option><option>USD</option></select><label>Forma de pago</label><select name="forma_pago"><option>Efectivo</option><option>Tarjeta</option><option>Transferencia</option></select><label>Fecha de pago</label><input type="date" name="fecha_pago"><label>Próximo pago</label><input type="date" name="proximo_pago"><label>Comprobante</label><input type="file" name="comprobante"><button>Guardar y generar recibo PDF</button></form><script>function filtrarExpedientesCobranza(){var sel=document.getElementById('cliente_id');var cliente=sel.value;var opt=sel.options[sel.selectedIndex];document.getElementById('monto_total').value=opt.dataset.total||'';document.getElementById('deuda_actual').value=opt.dataset.deuda||'';if(opt.dataset.moneda){document.getElementById('moneda').value=opt.dataset.moneda;}var exp=document.getElementById('expediente_id');for(var i=0;i<exp.options.length;i++){var o=exp.options[i];if(!o.value){o.hidden=false;continue;}o.hidden=(cliente && o.dataset.cliente!==cliente);}exp.value='';}function llenarMontoExpediente(){var exp=document.getElementById('expediente_id');var o=exp.options[exp.selectedIndex];if(o && o.value){document.getElementById('monto_total').value=o.dataset.total||'';if(o.dataset.moneda){document.getElementById('moneda').value=o.dataset.moneda;}}}</script></div><div class="card print-list"><h2>Lista de pagos realizados <button class="no-print" onclick="window.print()">Imprimir lista</button></h2><table><tr><th>Folio</th><th>Cliente</th><th>Expediente</th><th>Concepto</th><th>Abono</th><th>Saldo</th><th>Moneda</th><th>Estatus</th><th>Próximo pago</th><th class="no-print">Recibo</th></tr>{% for p in datos %}<tr><td>{{p.folio}}</td><td>{{p.cliente.nombre if p.cliente else ""}}</td><td>{{p.expediente.numero if p.expediente else ""}}</td><td>{{p.concepto}}</td><td>{{p.abono}}</td><td>{{p.saldo}}</td><td>{{p.moneda}}</td><td>{{p.estatus}}</td><td>{{p.proximo_pago}}</td><td class="no-print"><a href="/recibo/{{p.id}}">Ver</a> | <a href="/recibo-pdf/{{p.id}}">PDF</a></td></tr>{% endfor %}</table></div>""", clientes_info=clientes_info, expedientes=expedientes_activos, datos=Cobranza.query.order_by(Cobranza.id.desc()).all())
...
def eliminar_notificacion(id):
    ...
    db.session.delete(aviso)
    db.session.commit()
    flash("Aviso eliminado de tu bandeja.")
    return redirect("/notificaciones-internas")
