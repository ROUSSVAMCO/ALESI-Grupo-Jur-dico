...
def llenar_plantilla_contrato(contrato):
    ...
    prestador = (contrato.prestador_nombre or "ROSA ISELA VÁZQUEZ MEDINA").upper().replace("LIC. ", "").strip()
    cliente = (contrato.cliente_nombre or "").upper()
    ...
    replacements = [
        ("____ de __________________ del ______", f"{dia} de {mes} del {anio}"),
        ("____ de __________________ del ______.", f"{dia} de {mes} del {anio}."),
        ("________________________________________", prestador),
        ("________________________________________________", cliente),
        ("___________________", contrato.prestador_correo or "lic.rosavazquezm@gmail.com"),
        ("________________________________________________", contrato.prestador_telefono or "6645657298"),
        ("____________________________________________________________________________\n_______________", contrato.cliente_domicilio or ""),
        ("____________________________________", contrato.fecha_nacimiento or ""),
        ("________________________________________________", contrato.curp or ""),
        ("_________________________________________________", contrato.rfc or ""),
        ("__________________________________________", contrato.estado_civil or ""),
        ("____________________________________________", contrato.ocupacion or ""),
        ("____________________________________________", contrato.cliente_telefono or ""),
        ("_____________________________________", contrato.cliente_correo or ""),
        ("___________________________________", contrato.identificacion or ""),
        ("________________________________", contrato.numero_identificacion or ""),
        ("____________________________", contrato.asunto or ""),
        ("$____________________________ PESOS M.N", f"${total:,.2f} PESOS M.N"),
        ("(____________________________________________________ ___________________________________\nPESOS ____/100 MONEDA NACIONAL)", f"({numero_a_letra(total)})"),
        ("$____________________________ PESOS M.N", f"${inicio:,.2f} PESOS M.N"),
        ("(____________________________________________________ ___________________________________\nPESOS ____/100 MONEDA NACIONAL)", f"({numero_a_letra(inicio)})"),
        ("$____________________________ PESOS M.N", f"${abono:,.2f} PESOS M.N"),
        ("(____________________________________________________ ___________________________________\nPESOS ____/100 MONEDA NACIONAL)", f"({numero_a_letra(abono)})"),
        ("PAGADEROS _________________ y hasta la liquidación.", f"PAGADEROS {contrato.periodicidad_abono or ''} y hasta la liquidación."),
        ("LIC.\n________________________", "LIC.\n" + prestador),
        ("Nombre: ", "Nombre: " + cliente),
        ("Teléfono: ________________________", "Teléfono: " + (contrato.cliente_telefono or "")),
        ("Identificación Oficial No.\n_________", "Identificación Oficial No.\n" + (contrato.numero_identificacion or "")),
    ]
    for buscar, reemplazo in replacements:
        replace_first_in_runs(doc, buscar, reemplazo)
...
def contratos():
    ...
    if request.method == "POST":
        ...
        c = ContratoServicio(
            ...
            fecha_contrato=request.form.get("fecha_contrato") or datetime.now().strftime("%Y-%m-%d"),
    ...
    """, clientes=Cliente.query.order_by(Cliente.nombre).all(), expedientes=Expediente.query.order_by(Expediente.numero).all(), contratos=contratos_lista, hoy=datetime.now().strftime("%Y-%m-%d"))
...
def contrato_pdf(id):
    ...
    flash("El contrato se generó con la plantilla exacta, pero este servidor no tiene LibreOffice para convertirlo a PDF. Descarga el Word y usa Imprimir > Guardar como PDF.")
    return redirect("/contrato-descargar/" + str(c.id))
