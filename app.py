...
def contrato_word_response(cliente, expediente, datos):
    fecha = datos.get("fecha", datetime.now().strftime("%d DE %B DEL %Y")).upper()
    asunto = datos.get("asunto") or (expediente.tipo_asunto if expediente else cliente.tipo_asunto or "SERVICIOS JURÍDICOS PROFESIONALES")
    monto = datos.get("monto") or str(expediente.cobro_total if expediente and expediente.cobro_total else cliente.cobro_total or 0)
    monto_letra = datos.get("monto_letra") or "CANTIDAD PACTADA"
    forma_pago = datos.get("forma_pago") or "de conformidad con los pagos que acuerden las partes, hasta cubrir la totalidad de los honorarios pactados"
    abogado = datos.get("abogado") or "ALESI GRUPO JURÍDICO"
    telefono = cliente.telefono or ""
    html = f"""<html><head><meta charset='utf-8'><style>body{{font-family:Arial;line-height:1.45}} h1,h2{{text-align:center}} .firma{{margin-top:55px;display:flex;justify-content:space-between}} .firma div{{width:45%;text-align:center;border-top:1px solid #000;padding-top:8px}}</style></head><body><h1>CONTRATO DE PRESTACIÓN DE SERVICIOS JURÍDICOS PROFESIONALES</h1><h2>Inicio {fecha}</h2><p>A continuación, se presentan en términos resumidos los servicios profesionales que prestará <b>{abogado}</b> a <b>C. {cliente.nombre}</b>, respecto de <b>{asunto}</b>.</p><p>Por los servicios antes descritos, EL CLIENTE pagará la cantidad de <b>${monto} ({monto_letra})</b>, que se cubrirá de la siguiente manera: <b>{forma_pago}</b>.</p><p><b>PRIMERO.</b> EL CLIENTE manifiesta su conformidad con la contratación de los servicios jurídicos profesionales y autoriza el inicio de las gestiones, promociones, asesorías, escritos y actuaciones necesarias para la atención del asunto indicado.</p><p><b>SEGUNDO.</b> Los honorarios deberán cubrirse conforme a la forma de pago pactada. Los pagos realizados por transferencia, depósito o efectivo deberán conservarse mediante comprobante, recibo o constancia correspondiente.</p><p><b>TERCERO.</b> El costo pactado podrá variar por causas relacionadas con la estrategia, complejidad, duración del procedimiento, actuaciones extraordinarias o trámites adicionales, lo cual deberá informarse previamente al cliente.</p><p><b>CUARTO.</b> Los gastos por copias certificadas, actas, apostillas, traducciones, notificaciones, diligencias, traslados, derechos oficiales, servicios notariales o cualquier trámite institucional serán independientes de los honorarios, salvo pacto escrito en contrario.</p><h2>CLÁUSULAS</h2><p><b>Del desistimiento.</b> En caso de desistimiento, abandono o terminación por parte de EL CLIENTE respecto de los servicios profesionales, las cantidades pagadas hasta ese momento no serán objeto de devolución, al corresponder a trabajos, gestiones y tiempo profesional previamente realizado.</p><p>La devolución de documentos originales deberá solicitarse por escrito. Cuando los documentos obren en juzgado o autoridad, su devolución dependerá del trámite correspondiente y de los costos oficiales aplicables.</p><p><b>De la caducidad.</b> La falta de actividad, falta de pago, omisión de proporcionar documentos, datos, firmas o información requerida podrá afectar la continuidad del asunto. Si el cliente desea reactivar un asunto suspendido o caducado, podrán pactarse nuevos honorarios.</p><p><b>Del juicio o trámite.</b> La duración de cualquier procedimiento es estimada y depende de la autoridad, carga de trabajo, conducta procesal de las partes y naturaleza del asunto. Ningún resultado específico queda garantizado.</p><p>EL CLIENTE se obliga a proporcionar información veraz, documentos completos, datos de localización, firmas y apoyo necesario para la atención del asunto. La falsedad u omisión de información podrá ser causa de terminación del servicio.</p><p><b>Del pago.</b> El asunto deberá encontrarse cubierto conforme a lo pactado antes de la conclusión total, entrega de documentos finales, copias certificadas, convenio, sentencia o trámite final, salvo acuerdo distinto por escrito.</p><p>En caso de incumplimiento de pago por dos mensualidades o periodos consecutivos, EL DESPACHO podrá suspender gestiones no urgentes y ejercer las acciones legales que correspondan para garantizar el cumplimiento de las obligaciones pactadas.</p><h2>ACEPTACIÓN Y APROBACIÓN</h2><p>EL CLIENTE manifiesta estar de acuerdo con los términos del presente contrato de prestación de servicios profesionales y autoriza el uso de la documentación proporcionada únicamente para la atención del asunto señalado.</p><p>Tijuana, Baja California, a {fecha}.</p><div class='firma'><div>C. {cliente.nombre}<br>Tel. {telefono}</div><div>{abogado}</div></div></body></html>"""
    resp = Response(html, mimetype="application/msword")
    resp.headers["Content-Disposition"] = f"attachment; filename=contrato_{cliente.nombre.replace(' ', '_')}.doc"
    return resp
...
def contrato_cliente(id):
    ...
    if request.method == "POST":
        ...
        return contrato_word_response(c, expediente, request.form)
    return render("""<div class="card"><h2>Crear contrato de prestación de servicios</h2><form method="post"><label>Expediente / asunto</label><select name="expediente_id"><option value="">Usar datos generales del cliente</option>{% for e in exps %}<option value="{{e.id}}">{{e.numero}} - {{e.tipo_asunto}} - {{e.moneda_cobro}} {{e.cobro_total}}</option>{% endfor %}</select><label>Fecha de inicio</label><input name="fecha" value="{{fecha}}"><label>Asunto / servicio</label><input name="asunto" value="{{c.tipo_asunto or ''}}"><label>Monto</label><input name="monto" value="{{c.cobro_total or 0}}"><label>Monto con letra</label><input name="monto_letra" placeholder="Ej. TREINTA MIL PESOS 00/100 MONEDA NACIONAL"><label>Forma de pago</label><textarea name="forma_pago" placeholder="Ej. 50% de cada cantidad recuperada hasta cubrir honorarios, pago inicial, parcialidades, etc."></textarea><label>Nombre del despacho/abogado</label><input name="abogado" value="ALESI GRUPO JURÍDICO"><button>Descargar contrato en Word</button></form></div>""", c=c, exps=exps, fecha=datetime.now().strftime("%d DE %B DEL %Y").upper())
