...
class ContratoServicio(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    cliente_id = db.Column(db.Integer, db.ForeignKey("cliente.id"))
    expediente_id = db.Column(db.Integer, db.ForeignKey("expediente.id"))
    usuario_id = db.Column(db.Integer, db.ForeignKey("usuario.id"))
    cliente_nombre = db.Column(db.String(220))
    cliente_telefono = db.Column(db.String(80))
    cliente_correo = db.Column(db.String(180))
    cliente_domicilio = db.Column(db.Text)
    fecha_nacimiento = db.Column(db.String(40))
    curp = db.Column(db.String(80))
    rfc = db.Column(db.String(80))
    estado_civil = db.Column(db.String(80))
    ocupacion = db.Column(db.String(120))
    identificacion = db.Column(db.String(180))
    numero_identificacion = db.Column(db.String(120))
    prestador_nombre = db.Column(db.String(220), default="ROSA ISELA VÁZQUEZ MEDINA")
    prestador_correo = db.Column(db.String(180), default="lic.rosavazquezm@gmail.com")
    prestador_telefono = db.Column(db.String(80), default="6645657298")
    fecha_contrato = db.Column(db.String(20))
    asunto = db.Column(db.String(250))
    honorarios_total = db.Column(db.Float, default=0)
    honorarios_inicio = db.Column(db.Float, default=0)
    honorarios_abono = db.Column(db.Float, default=0)
    periodicidad_abono = db.Column(db.String(80))
    creado_en = db.Column(db.DateTime, default=datetime.now)
    cliente = db.relationship("Cliente")
    expediente = db.relationship("Expediente")
    usuario = db.relationship("Usuario")
...
nav a{color:white;text-decoration:none;margin-right:13px;font-weight:bold}
...
<div class="top">
    ...
    <div>{% if usuario %}{{usuario.nombre}} | {{usuario.rol}} | <a style="color:white" href="/logout">Salir</a>{% endif %}</div>
</div>
<section class="hero">
...
def mis_expedientes():
    if not req():
        return redirect("/login")
    datos = Expediente.query.filter_by(propietario_id=actual().id).order_by(Expediente.id.desc()).all()
    return render("""<div class="card no-print"><h2>Nuevo expediente</h2><p>Para capturar un nuevo expediente usa el botón siguiente.</p><a class="btn" href="/expedientes">Nuevo expediente</a></div><div class="card print-list"><h2>Mis Expedientes <button class="no-print" onclick="window.print()">Imprimir lista</button></h2><table><tr><th>Número</th><th>Cliente</th><th>Materia</th><th>Tipo</th><th>Estado</th><th class="no-print"></th></tr>{% for e in datos %}<tr><td>{{e.numero}}</td><td>{{e.cliente.nombre if e.cliente else ""}}</td><td>{{e.materia}}</td><td>{{e.tipo_asunto}}</td><td>{{e.estado}}</td><td class="no-print"><a class="btn btn2" href="/expediente/{{e.id}}">Abrir</a></td></tr>{% endfor %}</table></div>""", datos=datos)
...
# =========================
# CONTRATOS CON PLANTILLA EXACTA ALESI
# =========================
...
def numero_a_letra(n):
    ...
    def cientos(x):
        ...
        c, r = divmod(x, 100)
        return CENTENAS[c] if r == 0 else CENTENAS[c] + " " + cientos(r)
    ...
    if entero == 0:
        ...
        letras = "MIL" if miles == 1 else cientos(miles) + " MIL"
        if resto:
            letras += " " + cientos(resto)
    else:
        millones, resto = divmod(entero, 1000000)
        letras = "UN MILLÓN" if millones == 1 else cientos(millones) + " MILLONES"
        if resto:
            if resto < 1000:
                letras += " " + cientos(resto)
            else:
                miles, ult = divmod(resto, 1000)
                letras += " " + ("MIL" if miles == 1 else cientos(miles) + " MIL")
                if ult:
                    letras += " " + cientos(ult)
    return f"{letras} PESOS {centavos:02d}/100 MONEDA NACIONAL"


def fecha_contrato_partes(fecha_txt):
    try:
        d = datetime.strptime(fecha_txt, "%Y-%m-%d")
    except Exception:
        d = local_now()
    return str(d.day).zfill(2), MESES_ES.get(d.month, ""), str(d.year)


def plantilla_contrato_path():
    posibles = [
        "contrato_alesi.docx",
...
def replace_first_in_runs(doc, buscar, reemplazo):
    """Reemplaza la primera coincidencia de texto dentro de párrafos o tablas conservando el estilo del run."""
    reemplazo = str(reemplazo or "")
    ...
    for p in contenedores:
        ...
        if buscar in texto:
            nuevo = texto.replace(buscar, reemplazo, 1)
            if p.runs:
                p.runs[0].text = nuevo
                for r in p.runs[1:]:
                    r.text = ""
...
def llenar_plantilla_contrato(contrato):
    try:
        from docx import Document
    except Exception as exc:
        raise RuntimeError("Falta instalar python-docx. Agrega python-docx==1.1.2 en requirements.txt") from exc

    plantilla = plantilla_contrato_path()
    if not plantilla:
        raise FileNotFoundError("No encontré la plantilla contrato_alesi.docx en el proyecto.")

    doc = Document(plantilla)
    dia, mes, anio = fecha_contrato_partes(contrato.fecha_contrato)

    prestador = (contrato.prestador_nombre or "ROSA ISELA VÁZQUEZ MEDINA").replace("LIC.", "").strip().upper()
    cliente = (contrato.cliente_nombre or "").strip().upper()
    total = float(contrato.honorarios_total or 0)
    inicio = float(contrato.honorarios_inicio or 0)
    abono = float(contrato.honorarios_abono or 0)

    total_letra = numero_a_letra(total)
    inicio_letra = numero_a_letra(inicio)
    abono_letra = numero_a_letra(abono)

    # Esta función usa TU MISMA PLANTILLA y llena los espacios exactos por ubicación de párrafo.
    # No cambia encabezados, logos, márgenes, colores ni diseño.
    p = doc.paragraphs

    set_paragraph_text(p[2], f"Tijuana, Baja California, a {dia} de {mes} del {anio}.")
    set_paragraph_text(p[4], f'CONTRATO DE PRESTACIÓN DE SERVICIOS JURÍDICOS PROFESIONALES QUE CELEBRAN POR UNA PARTE EL (LA) LICENCIADA EN DERECHO {prestador}, A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ "EL PRESTADOR"; Y POR LA OTRA PARTE EL(LA) C. {cliente}, A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ "EL CLIENTE", QUIENES MANIFIESTAN SU VOLUNTAD DE OBLIGARSE AL TENOR DE LAS SIGUIENTES:')

    set_paragraph_text(p[11], "Correo electrónico: " + (contrato.prestador_correo or "lic.rosavazquezm@gmail.com"))
    set_paragraph_text(p[12], "Teléfono: " + (contrato.prestador_telefono or "6645657298"))

    set_paragraph_text(p[17], "d) Que señala como domicilio para oír y recibir notificaciones el ubicado en: " + (contrato.cliente_domicilio or ""))
    set_paragraph_text(p[19], "Fecha de Nacimiento: " + (contrato.fecha_nacimiento or ""))
    set_paragraph_text(p[20], "CURP: " + (contrato.curp or ""))
    set_paragraph_text(p[21], "RFC: " + (contrato.rfc or ""))
    set_paragraph_text(p[22], "Estado Civil: " + (contrato.estado_civil or ""))
    set_paragraph_text(p[23], "Ocupación: " + (contrato.ocupacion or ""))
    set_paragraph_text(p[24], "Teléfono: " + (contrato.cliente_telefono or ""))
    set_paragraph_text(p[25], "Correo Electrónico: " + (contrato.cliente_correo or ""))
    set_paragraph_text(p[26], "Identificación Oficial: " + (contrato.identificacion or ""))
    set_paragraph_text(p[27], "Número de Identificación: " + (contrato.numero_identificacion or ""))

    set_paragraph_text(p[35], "EL PRESTADOR se obliga a proporcionar a EL CLIENTE servicios jurídicos profesionales consistentes en asesoría, orientación legal, elaboración de documentos jurídicos, representación, gestión administrativa, negociación, mediación, conciliación, seguimiento procesal y, en su caso, litigio ante autoridades administrativas, judiciales o de cualquier otra naturaleza, respecto del siguiente asunto: " + (contrato.asunto or ""))

    set_paragraph_text(p[41], f"Por los servicios profesionales objeto del presente contrato, EL CLIENTE se obliga a pagar a EL PRESTADOR la cantidad de: ${total:,.2f} PESOS M.N ({total_letra}) por concepto de HONORARIOS")
    set_paragraph_text(p[43], f"${inicio:,.2f} PESOS M.N ({inicio_letra}) por concepto DE INICIO, pagaderos a la firma del presente contrato.")
    set_paragraph_text(p[44], f"${abono:,.2f} PESOS M.N ({abono_letra}) POR CONCEPTO DE ABONO PAGADEROS {contrato.periodicidad_abono or ''} y hasta la liquidación.")

    set_paragraph_text(p[106], f"Leído que fue el presente contrato y enteradas las partes de su contenido, alcance y consecuencias legales, lo firman por duplicado en la ciudad de Tijuana, Baja California, a {dia} de {mes} del {anio}.")
    set_paragraph_text(p[111], f"{prestador}                                                        Nombre: {cliente}")
    set_paragraph_text(p[112], "Teléfono: " + (contrato.cliente_telefono or ""))
    set_paragraph_text(p[113], "Identificación Oficial No. " + (contrato.numero_identificacion or ""))
...
def contratos():
    ...
    if request.method == "POST":
        cliente_id = request.form.get("cliente_id") or None
        expediente_id = request.form.get("expediente_id") or None
        cliente_obj = db.session.get(Cliente, int(cliente_id)) if cliente_id else None
        c = ContratoServicio(
            cliente_id=cliente_id,
            expediente_id=expediente_id,
            usuario_id=actual().id,
            cliente_nombre=request.form.get("cliente_nombre") or (cliente_obj.nombre if cliente_obj else ""),
            cliente_telefono=request.form.get("cliente_telefono") or (cliente_obj.telefono if cliente_obj else ""),
            cliente_correo=request.form.get("cliente_correo") or (cliente_obj.correo if cliente_obj else ""),
            cliente_domicilio=request.form.get("cliente_domicilio") or (cliente_obj.direccion if cliente_obj else ""),
            fecha_nacimiento=request.form.get("fecha_nacimiento", ""),
            curp=request.form.get("curp") or (cliente_obj.curp if cliente_obj else ""),
            rfc=request.form.get("rfc") or (cliente_obj.rfc if cliente_obj else ""),
            estado_civil=request.form.get("estado_civil", ""),
            ocupacion=request.form.get("ocupacion", ""),
            identificacion=request.form.get("identificacion", ""),
            numero_identificacion=request.form.get("numero_identificacion", ""),
            prestador_nombre=(request.form.get("prestador_nombre") or "ROSA ISELA VÁZQUEZ MEDINA").replace("LIC. ", "").upper(),
            prestador_correo=request.form.get("prestador_correo") or "lic.rosavazquezm@gmail.com",
            prestador_telefono=request.form.get("prestador_telefono") or "6645657298",
            fecha_contrato=request.form.get("fecha_contrato") or local_today_str(),
            asunto=request.form.get("asunto", ""),
            honorarios_total=float(request.form.get("honorarios_total") or 0),
            honorarios_inicio=float(request.form.get("honorarios_inicio") or 0),
            honorarios_abono=float(request.form.get("honorarios_abono") or 0),
            periodicidad_abono=request.form.get("periodicidad_abono", ""),
        )
        db.session.add(c)
        db.session.commit()
        flash("Contrato guardado. Descárgalo para imprimirlo.")
        return redirect("/contrato-pdf/" + str(c.id))
    contratos_lista = ContratoServicio.query.order_by(ContratoServicio.id.desc()).all()
    return render("""
    <div class="grid2">
    <div class="card">
    <h2>Generar contrato con plantilla ALESI</h2>
    <p>Se usa la plantilla Word exacta del contrato. Solo se llenan los espacios en blanco.</p>
    <form method="post">
        <label>Cliente existente</label><select name="cliente_id" onchange="llenarClienteContrato(this)"><option value="">Nuevo / capturar manual</option>{% for c in clientes %}<option value="{{c.id}}" data-nombre="{{c.nombre}}" data-telefono="{{c.telefono or ''}}" data-correo="{{c.correo or ''}}" data-domicilio="{{c.direccion or ''}}" data-rfc="{{c.rfc or ''}}" data-curp="{{c.curp or ''}}">{{c.nombre}}</option>{% endfor %}</select>
        <label>Expediente</label><select name="expediente_id"><option value="">Sin expediente</option>{% for e in expedientes %}<option value="{{e.id}}">{{e.numero}} - {{e.cliente.nombre if e.cliente else ''}}</option>{% endfor %}</select>
        <label>Fecha del contrato</label><input type="date" name="fecha_contrato" value="{{hoy}}">
        <label>Nombre del cliente</label><input id="cliente_nombre" name="cliente_nombre" required>
        <label>Teléfono del cliente</label><input id="cliente_telefono" name="cliente_telefono">
        <label>Correo del cliente</label><input id="cliente_correo" name="cliente_correo">
        <label>Domicilio del cliente</label><textarea id="cliente_domicilio" name="cliente_domicilio"></textarea>
        <label>Fecha de nacimiento</label><input name="fecha_nacimiento">
        <label>CURP</label><input id="curp" name="curp">
        <label>RFC</label><input id="rfc" name="rfc">
        <label>Estado civil</label><input name="estado_civil">
        <label>Ocupación</label><input name="ocupacion">
        <label>Identificación oficial</label><input name="identificacion" placeholder="INE, pasaporte, licencia, etc.">
        <label>Número de identificación</label><input name="numero_identificacion">
        <label>Nombre del prestador</label><input name="prestador_nombre" value="ROSA ISELA VÁZQUEZ MEDINA">
        <label>Correo del prestador</label><input name="prestador_correo" value="lic.rosavazquezm@gmail.com">
        <label>Teléfono del prestador</label><input name="prestador_telefono" value="6645657298">
        <label>Asunto jurídico</label><input name="asunto" required>
        <label>Honorarios totales</label><input type="number" step="0.01" name="honorarios_total" required>
        <label>Pago inicial</label><input type="number" step="0.01" name="honorarios_inicio" value="0">
        <label>Abono</label><input type="number" step="0.01" name="honorarios_abono" value="0">
        <label>Periodicidad del abono</label><select name="periodicidad_abono"><option>SEMANAL</option><option>QUINCENAL</option><option>MENSUAL</option><option>SEGÚN ACUERDO DE LAS PARTES</option></select>
        <button>Guardar y descargar contrato</button>
    </form>
    <script>
    function llenarClienteContrato(sel){
        var o=sel.options[sel.selectedIndex];
        document.getElementById('cliente_nombre').value=o.dataset.nombre||'';
        document.getElementById('cliente_telefono').value=o.dataset.telefono||'';
        document.getElementById('cliente_correo').value=o.dataset.correo||'';
        document.getElementById('cliente_domicilio').value=o.dataset.domicilio||'';
        document.getElementById('rfc').value=o.dataset.rfc||'';
        document.getElementById('curp').value=o.dataset.curp||'';
    }
    </script>
    </div>
    <div class="card">
    <h2>Contratos guardados</h2>
    <table><tr><th>Fecha</th><th>Cliente</th><th>Asunto</th><th>Total</th><th>Descargar</th></tr>{% for c in contratos %}<tr><td>{{c.fecha_contrato}}</td><td>{{c.cliente_nombre}}</td><td>{{c.asunto}}</td><td>{{c.honorarios_total}}</td><td><a class="btn btn2" href="/contrato-descargar/{{c.id}}">Word</a> <a class="btn" href="/contrato-pdf/{{c.id}}">PDF</a></td></tr>{% endfor %}</table>
    </div>
    </div>
    """, clientes=Cliente.query.order_by(Cliente.nombre).all(), expedientes=Expediente.query.order_by(Expediente.numero).all(), contratos=contratos_lista, hoy=local_today_str())


@app.route("/contrato-descargar/<int:id>")
def contrato_descargar(id):
    if not req():
        return redirect("/login")
    c = ContratoServicio.query.get_or_404(id)
    try:
        archivo = llenar_plantilla_contrato(c)
    except Exception as exc:
        flash(str(exc))
        return redirect("/contratos")
    nombre = "contrato_" + (c.cliente_nombre or "cliente").replace(" ", "_") + ".docx"
    return send_file(archivo, mimetype="application/vnd.openxmlformats-officedocument.wordprocessingml.document", as_attachment=True, download_name=nombre)


@app.route("/contrato-pdf/<int:id>")
def contrato_pdf(id):
    if not req():
        return redirect("/login")
    c = ContratoServicio.query.get_or_404(id)
    ...
    # Para conservar el contrato exactamente igual, primero se genera el Word con la plantilla real.
    # Si Render tiene LibreOffice instalado, se convierte a PDF; si no, se entrega el Word exacto.
    with tempfile.TemporaryDirectory() as tmp:
        docx_path = os.path.join(tmp, "contrato.docx")
        with open(docx_path, "wb") as f:
            f.write(archivo.getvalue())
        soffice = shutil.which("soffice") or shutil.which("libreoffice")
        if soffice:
            subprocess.run([soffice, "--headless", "--convert-to", "pdf", "--outdir", tmp, docx_path], check=False, timeout=60)
            pdf_path = os.path.join(tmp, "contrato.pdf")
            if os.path.exists(pdf_path):
                return send_file(pdf_path, mimetype="application/pdf", as_attachment=True, download_name="contrato_" + str(c.id) + ".pdf")
    flash("El contrato se llenó correctamente con tu plantilla exacta, pero Render necesita LibreOffice para descargarlo como PDF. Sube un archivo Aptfile con la palabra libreoffice y vuelve a desplegar.")
    return redirect("/contrato-descargar/" + str(c.id))
