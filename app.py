...
def contrato_pdf_directo_response(cliente, expediente, datos):
    """Genera el contrato directamente en PDF, sin guardar registro y sin depender de LibreOffice."""
    cliente_nombre = (datos.get("cliente_nombre") or cliente.nombre or "").upper()
    cliente_tel = datos.get("telefono") or cliente.telefono or ""
    cliente_correo = datos.get("correo") or cliente.correo or ""
    domicilio = datos.get("domicilio") or cliente.direccion or ""
    ...
    periodicidad = datos.get("periodicidad_abono") or ""
    forma_pago = datos.get("forma_pago") or ""
    prestador = limpiar_prestador(datos.get("prestador") or "ROSA ISELA VÁZQUEZ MEDINA")
    prestador_correo = datos.get("prestador_correo") or "lic.rosavazquezm@gmail.com"
    prestador_telefono = datos.get("prestador_telefono") or "6645657298"
    fecha_iso = datos.get("fecha_contrato") or local_date_iso()
    dia, mes, anio, fecha_larga = fecha_contrato_texto(fecha_iso)
    ...
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter, rightMargin=48, leftMargin=48, topMargin=96, bottomMargin=68)
    styles = getSampleStyleSheet()
    normal = ParagraphStyle('ContratoNormal', parent=styles['Normal'], fontName='Times-Roman', fontSize=10.5, leading=13, alignment=4)
    bold = ParagraphStyle('ContratoBold', parent=normal, fontName='Times-Bold')
    title = ParagraphStyle('ContratoTitle', parent=styles['Title'], fontName='Times-Bold', fontSize=13, leading=16, alignment=1)
    center = ParagraphStyle('ContratoCenter', parent=normal, alignment=1)
    small = ParagraphStyle('ContratoSmall', parent=normal, fontSize=9, leading=11)

    story = []
    story.append(Paragraph('CONTRATO DE PRESTACIÓN DE SERVICIOS JURÍDICOS PROFESIONALES', title))
    story.append(Spacer(1, 8))
    story.append(Paragraph(f'Tijuana, Baja California, a {dia} de {mes} del {anio}.', center))
    story.append(Spacer(1, 12))
    story.append(Paragraph(f'CONTRATO DE PRESTACIÓN DE SERVICIOS JURÍDICOS PROFESIONALES QUE CELEBRAN POR UNA PARTE {prestador}, A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ "EL PRESTADOR"; Y POR LA OTRA PARTE EL(LA) C. {cliente_nombre}, A QUIEN EN LO SUCESIVO SE LE DENOMINARÁ "EL CLIENTE", QUIENES MANIFIESTAN SU VOLUNTAD DE OBLIGARSE AL TENOR DE LAS SIGUIENTES:', normal))
    story.append(Spacer(1, 10))
    story.append(Paragraph('DECLARACIONES', title))
    story.append(Paragraph('I. DECLARA EL PRESTADOR:', bold))
    story.append(Paragraph('a) Que es profesionista legalmente autorizada para ejercer la profesión de Licenciada en Derecho, contando con la preparación académica, experiencia y capacidad técnica necesarias para prestar servicios de asesoría, consultoría, representación y litigio en materia jurídica.', normal))
    story.append(Paragraph('b) Que tiene la capacidad legal suficiente para celebrar el presente contrato y obligarse en los términos del mismo.', normal))
    story.append(Paragraph('c) Que señala como domicilio para oír y recibir notificaciones el ubicado en calle Cuauhtémoc 5048, colonia Ampliación Gabriel Rodríguez, 22207, Tijuana, Baja California.', normal))
    story.append(Paragraph(f'd) Que su correo electrónico y teléfono de contacto son: Correo electrónico: {prestador_correo}. Teléfono: {prestador_telefono}.', normal))
    story.append(Spacer(1, 8))
    story.append(Paragraph('II. DECLARA EL CLIENTE:', bold))
    story.append(Paragraph('a) Que es una persona física con capacidad legal suficiente para contratar y obligarse en los términos del presente instrumento.', normal))
    story.append(Paragraph('b) Que requiere los servicios profesionales de EL PRESTADOR para la atención de un asunto jurídico de su interés.', normal))
    story.append(Paragraph('c) Que la información y documentación que proporcione para la atención del asunto es auténtica, veraz y vigente.', normal))
    story.append(Paragraph(f'd) Que señala como domicilio para oír y recibir notificaciones el ubicado en: {domicilio}.', normal))
    story.append(Paragraph(f'e) Que proporciona los siguientes datos de identificación: Fecha de Nacimiento: {fecha_nacimiento}. CURP: {curp}. RFC: {rfc}. Estado Civil: {estado_civil}. Ocupación: {ocupacion}. Teléfono: {cliente_tel}. Correo Electrónico: {cliente_correo}. Identificación Oficial: {identificacion}. Número de Identificación: {numero_identificacion}.', normal))
    story.append(Spacer(1, 8))
    story.append(Paragraph('III. DECLARAN AMBAS PARTES:', bold))
    story.append(Paragraph('ÚNICA. - Que se reconocen mutuamente la personalidad con la que comparecen, así como la capacidad legal necesaria para obligarse conforme a los términos y condiciones establecidos en el presente contrato.', normal))
    story.append(Spacer(1, 10))
    story.append(Paragraph('CLÁUSULAS', title))
    clausulas = [
        ('PRIMERA. - OBJETO. -', f'EL PRESTADOR se obliga a proporcionar a EL CLIENTE servicios jurídicos profesionales consistentes en asesoría, orientación legal, elaboración de documentos jurídicos, representación, gestión administrativa, negociación, mediación, conciliación, seguimiento procesal y, en su caso, litigio ante autoridades administrativas, judiciales o de cualquier otra naturaleza, respecto del siguiente asunto: {asunto}. La prestación de servicios comprenderá únicamente las actuaciones relacionadas con el asunto anteriormente descrito.'),
        ('SEGUNDA. - ALCANCE DE LOS SERVICIOS. -', 'Los servicios objeto del presente contrato comprenden todas aquellas actividades profesionales que resulten razonablemente necesarias para la adecuada atención del asunto encomendado. Cualquier procedimiento, recurso, incidente, juicio diverso, apelación, amparo, ejecución de sentencia o trámite distinto al originalmente contratado requerirá autorización expresa del cliente y podrá generar honorarios adicionales.'),
        ('TERCERA. - HONORARIOS PROFESIONALES. -', f'Por los servicios profesionales objeto del presente contrato, EL CLIENTE se obliga a pagar a EL PRESTADOR la cantidad de: ${monto_total:,.2f} PESOS M.N. ({cantidad_en_letra(monto_total)}) por concepto de HONORARIOS. Los honorarios serán cubiertos de la siguiente forma: ${monto_inicio:,.2f} PESOS M.N. ({cantidad_en_letra(monto_inicio)}) por concepto DE INICIO, pagaderos a la firma del presente contrato. ${monto_abono:,.2f} PESOS M.N. ({cantidad_en_letra(monto_abono)}) POR CONCEPTO DE ABONO PAGADEROS {periodicidad} y hasta la liquidación. {forma_pago} La falta de pago de cualquiera de las parcialidades en las fechas convenidas constituirá mora automática, sin necesidad de requerimiento judicial o extrajudicial.'),
        ('CUARTA. - GASTOS Y COSTAS. -', 'Los honorarios pactados no incluyen gastos extraordinarios necesarios para la atención del asunto, incluyendo de manera enunciativa más no limitativa: notificaciones, emplazamientos, edictos, derechos gubernamentales, certificaciones, copias certificadas, honorarios de fedatarios públicos, traducciones, apostillas, peritajes, gastos de ejecución, publicaciones, viáticos, traslados y diligencias fuera de la ciudad. Dichos gastos serán cubiertos por EL CLIENTE previa solicitud o comprobación correspondiente.'),
        ('QUINTA. - OBLIGACIONES DEL CLIENTE. -', 'EL CLIENTE se obliga a proporcionar información completa, verdadera y oportuna; entregar la documentación requerida; comparecer personalmente cuando sea necesaria su presencia; cubrir oportunamente los honorarios y gastos; mantener actualizados sus datos de localización e informar cualquier circunstancia que pueda afectar el desarrollo del asunto.'),
        ('SEXTA. - OBLIGACIONES DEL PRESTADOR. -', 'EL PRESTADOR se obliga a actuar con diligencia, ética profesional y apego a derecho; mantener informado al cliente sobre el estado procesal del asunto; guardar absoluta confidencialidad respecto de la información proporcionada; y realizar las actuaciones profesionales necesarias para la adecuada atención del asunto.'),
        ('SÉPTIMA. - NATURALEZA DE LA OBLIGACIÓN. -', 'Las partes reconocen que los servicios objeto del presente contrato corresponden a una prestación de servicios profesionales de carácter jurídico, por lo que las obligaciones asumidas por EL PRESTADOR son de medios y no de resultados. En consecuencia, EL PRESTADOR se compromete a emplear sus conocimientos, experiencia, capacidad profesional y los recursos jurídicos legalmente procedentes para la adecuada atención del asunto encomendado; sin embargo, no garantiza el sentido de las resoluciones, sentencias, acuerdos o determinaciones que emitan las autoridades administrativas, jurisdiccionales o de cualquier otra naturaleza que intervengan en el asunto.'),
        ('OCTAVA. - CONFIDENCIALIDAD. -', 'Toda información, documentación, datos personales y antecedentes proporcionados por EL CLIENTE tendrán carácter estrictamente confidencial y serán utilizados exclusivamente para la atención del asunto objeto del presente contrato.'),
        ('NOVENA. - TERMINACIÓN ANTICIPADA. -', 'El presente contrato podrá darse por terminado anticipadamente por cualquiera de las partes mediante aviso por escrito. En caso de terminación por decisión de EL CLIENTE, los honorarios ya pagados no serán reembolsables y deberán cubrirse aquellos servicios efectivamente prestados a la fecha de terminación. La terminación anticipada no extingue las obligaciones de pago pendientes a cargo de EL CLIENTE.'),
        ('DÉCIMA. - RESCISIÓN. -', 'EL PRESTADOR podrá rescindir el presente contrato sin responsabilidad alguna cuando EL CLIENTE proporcione información falsa, oculte información relevante para el asunto, exista incumplimiento en el pago de honorarios o gastos, o se generen conflictos que imposibiliten la adecuada representación profesional.'),
        ('DÉCIMA PRIMERA. - INCUMPLIMIENTO DE PAGO. -', 'Si EL CLIENTE incumple con el pago de los honorarios pactados por un periodo mayor a treinta días naturales, EL PRESTADOR podrá suspender la prestación de los servicios hasta que el adeudo sea cubierto en su totalidad. Lo anterior será independiente de las acciones legales procedentes para el cobro de los adeudos existentes.'),
        ('DÉCIMA SEGUNDA. - DOCUMENTACIÓN. -', 'EL CLIENTE autoriza a EL PRESTADOR para utilizar la documentación proporcionada exclusivamente para la atención del asunto descrito en este contrato. Los documentos originales podrán ser devueltos previa solicitud por escrito del cliente. EL CLIENTE manifiesta bajo protesta de decir verdad que la documentación entregada es auténtica.'),
        ('DÉCIMA TERCERA. - PROTECCIÓN DE DATOS PERSONALES. -', 'Las partes reconocen que los datos personales recabados con motivo de la celebración del presente contrato serán tratados conforme a la legislación aplicable en materia de protección de datos personales.'),
        ('DÉCIMA CUARTA. - JURISDICCIÓN. -', 'Para la interpretación, cumplimiento y ejecución del presente contrato, las partes se someten expresamente a las leyes y tribunales competentes de la ciudad de Tijuana, Baja California, renunciando a cualquier otro fuero que pudiera corresponderles por razón de sus domicilios presentes o futuros.'),
    ]
    for titulo, cuerpo in clausulas:
        story.append(Spacer(1, 6))
        story.append(Paragraph(titulo, bold))
        story.append(Paragraph(cuerpo, normal))

    story.append(Spacer(1, 14))
    story.append(Paragraph(f'Leído que fue el presente contrato y enteradas las partes de su contenido, alcance y consecuencias legales, lo firman por duplicado en la ciudad de Tijuana, Baja California, a {dia} de {mes} del {anio}.', normal))
    story.append(Spacer(1, 28))
    data = [[Paragraph('EL PRESTADOR', center), Paragraph('EL CLIENTE', center)],
            [Paragraph('______________________________', center), Paragraph('______________________________', center)],
            [Paragraph(prestador, center), Paragraph('Nombre: ' + cliente_nombre, center)],
            [Paragraph('', center), Paragraph('Teléfono: ' + cliente_tel, center)],
            [Paragraph('', center), Paragraph('Identificación Oficial No. ' + numero_identificacion, center)]]
    tabla = Table(data, colWidths=[240, 240])
    tabla.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'TOP'),('ALIGN',(0,0),(-1,-1),'CENTER'),('FONTNAME',(0,0),(-1,-1),'Times-Roman'),('FONTSIZE',(0,0),(-1,-1),10)]))
    story.append(tabla)

    doc.build(story, onFirstPage=encabezado_pie, onLaterPages=encabezado_pie)
    buffer.seek(0)
    nombre_base = 'contrato_' + cliente_nombre.replace(' ', '_').replace('/', '_') + '.pdf'
    return send_file(buffer, as_attachment=True, download_name=nombre_base, mimetype='application/pdf')
...
def contrato_cliente(id):
    ...
    if request.method == "POST":
        ...
        return contrato_pdf_directo_response(c, expediente, request.form)
    fecha_hoy = local_date_iso()
    return render("""
    <div class="card">
        <h2>Crear contrato de prestación de servicios jurídicos profesionales</h2>
        <p>Llena los datos y al presionar el botón se descargará el contrato en PDF listo para imprimir.</p>
        <form method="post">
            <label>Cliente</label>
            ...
            <button>Descargar contrato PDF</button>
        </form>
    </div>
    <script>
